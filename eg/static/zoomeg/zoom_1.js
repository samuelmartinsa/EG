...
l'image à zoomer est identifiée par <img class="cnxzoom" ...
voir notamment:
...
cnx.zoom = (function(){ //conteneur classe
	...
	return { // public section
		...
		mouseover: function(e){ // identifie image et crée la copie {{{
			if (cnx.zoom.iszoom)	return false; //les div existe déjà
			...
			if (cnx.style().isClassName("cnxzoom", cible)){ //l'image doit effectivement avoir un effet zoom -teste le style class cnxzoom
				...
				cnx.zoom.iszoom = cible; //mémorise l'image cible
				
				/* recherche des dimensions et position de l'image cible */
				var realWidth = cnx.style().getTrueWidth(cible); //taille réelle de l'image
				var realHeight = cnx.style().getTrueHeight(cible); //taille réelle de l'image
				...
				cnx.zoom.ratio = realWidth / cnx.zoom.actualWidth; //ratio entre taille réelle et taille actuelle de l'image à zoomer
				cnx.zoom.realLeft = cnx.style().getTrueLeft(cible); //position réelle de l'image
				cnx.zoom.realTop = cnx.style().getTrueTop(cible); //position réelle de l'image
				var decalage = 20; //décallage entre image actuelle et sa copie pour le zoom
				//if (cnx.style().isClassName("cnxleft", cible)) var imagecopieàgauche = true;
				...
				/*var copieLeft = (imagecopieàgauche) ? cnx.zoom.realLeft - cnx.zoom.actualWidth - decalage : cnx.zoom.realLeft + cnx.zoom.actualWidth + decalage; //position de la copie de l'image pour le zoom
				...
				if (copieDiv == null) { //créer la <div> si elle n'existe pas encore
					...
					cnx.dom().appendTo(copieDiv, copieImg); //insère l'image en taille réelle dans la div copie
					cnx.dom().appendTo(document.body, copieDiv); //document.body.appendChild(copieDiv); //insère la div copie dans la page web
				}
				...
				if (curseurDiv == null) { //créer la <div> si elle n'existe pas encore
					...
					cnx.style().setOpacity(30, curseurDiv); //opacité à 30% - 0 => transparence totale - pour cet effet il faut un style type backgroundColor
				}
		...
		mouseout: function(e){ // ne détecte pas le out lors d'un mousewheel d'où la vérification dans mousemove {{{
			var cible = cnx.event().getTarget(e);
			if(cible.id == "id_cnx_divcurseur")	cnx.zoom.raz(); //teste si nous avons quitté la div curseur - efface tous les paramètres
			return false;
		}, // }}}
		
		raz: function(){ // efface les paramètres si la souris sort de l'image cible{{{
			cnx.dom().removeElt("id_cnx_copieimg");//détruit image copie - taille réelle
			cnx.dom().removeElt("id_cnx_copiediv");//détruit div copie
			cnx.dom().removeElt("id_cnx_divcurseur"); //détruit la div autour du curseur de la souris
			
			cnx.zoom.iszoom = false; //contient l'image cible dont nous devons faire un effet zoom
			cnx.zoom.ratio = false; //ratio entre taille actuelle et taille réelle de l'image
			...
			cnx.zoom.realLeft = false; //postion réelle de l'image cible par rapport à la page web
			cnx.zoom.realTop = false; //postion réelle de l'image cible par rapport à la page web
		}, // }}}
			...
			if (cnx.zoom.iszoom){ // teste si image zoom existe
				...
				*/
				...
				if (cible.id != "id_cnx_divcurseur" && cible != cnx.zoom.iszoom){ //je ne suis plus au-dessus de l'image - utile lors d'un mousewheel
					cnx.zoom.raz(); //efface tous les paramètres
					return false; // je quitte le programme
				}
				
				var cible = cnx.zoom.iszoom;
				x = cnx.event().pageScrollX(e); //left - position de la souris à l'écran
				y = cnx.event().pageScrollY(e); //top - position de la souris à l'écran
				...
				var maxTop = cnx.zoom.realTop + cnx.zoom.actualHeight - dim + 2; //2 car cadre épaisseur 1px tout autour soit 1 en haut, 1 en bas => 2
				...
				//cnx.surv("img ("+ cnx.zoom.realLeft +","+cnx.zoom.realTop + ") souris ("+x + ","+y+")"); //ligne de contrôle - alert()
				...
				var dLeft = (relatif_left*(cnx.zoom.ratio-1)); //delta du déplacement de l'image en taille réelle
...
cnx.event().addEvent("mouseover", cnx.zoom.mouseover, document); //onmouseover recherche image à zoomer
cnx.event().addEvent("mousemove", cnx.zoom.mousemove, document); //onmousemove fait deffiler l'image portant le zoom
cnx.event().addEvent("mouseout", cnx.zoom.mouseout, document); //onmouseout détruit les données mémorisée pour annuler le zoom
